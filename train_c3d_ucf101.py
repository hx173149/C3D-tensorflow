...
def tower_loss(name_scope,logit,labels):
  cross_entropy_mean = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logit, labels))
  ...
  with tf.Graph().as_default():
    ...
    for gpu_index in range(0,gpu_num):
      with tf.device('/gpu:%d' % gpu_index):
        with tf.name_scope('%s_%d' % ('houxin', gpu_index)) as scope:
          with tf.variable_scope('var_name') as var_scope:
            weights = {
              ...
              'out': _variable_with_weight_decay('wout',[4096, model_build.NUM_CLASSES],0.0005)
              }
            biases = {
              ...
              'out': _variable_with_weight_decay('bout',[model_build.NUM_CLASSES],0.000),
              }
          varlist1 = weights.values()
          varlist2 = biases.values()
          logit = model_build.inference_c3d(images_placeholder[gpu_index*FLAGS.batch_size:(gpu_index+1)*FLAGS.batch_size,:,:,:,:],
                  0.5,FLAGS.batch_size,weights,biases) 
          loss = tower_loss(scope,logit,
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if (step) % 10 == 0 or (step + 1) == FLAGS.max_steps:
        saver.save(sess, 'models/c3d_ucf_model', global_step=step)
        ...
        val_images,val_labels = input_data.ReadDataLabelFromFile('list/test.list',FLAGS.batch_size*gpu_num)
